...
function load_mailbox(mailbox) {
  ...
  fetch(`/emails/${mailbox}`)
    ...
    .then((emails) => {
        ...
        tb.className=`table`;
        tb.innerHTML=`<table border='1' id='table_view'>
        <tr>
        <th>Subject</th>
        <th>Recipient</th>
        <th>Time</th>
        ...
        document.querySelector("#emails-view").appendChild(tb);
      ...
      emails.forEach((element) => {
        ...
        row.className   =   `${is_read} my-1 items email_h`;
        var td1 = document.createElement("td");
        var td2 = document.createElement("td");
        var td3 =   document.createElement("td");
        var cell_subject = document.createTextNode(`${element.subject}`);
        var cell_sender = document.createTextNode(`${sender_recipients}`);
        var cell_time   =   document.createTextNode(`${element.timestamp}`);
        td1.appendChild(cell_subject);
        td2.appendChild(cell_sender);
        td3.appendChild(cell_time);
        row.appendChild(td1);
        row.appendChild(td2);
        row.appendChild(td3);
        tb.appendChild(row);
...
/*
         ...
         item.innerHTML = `<div class="card-body email_h" id="item-${element.id}">
...
function show_mail(id, mailbox) {
  ...
  <br>${email.body}
      ...
      archive.className = `btn btn-sm btn-outline-info my-2`;
      ...
      reply.className = `btn btn-sm btn-outline-success m-2`;
